...
def build_selection(raw: dict, cards: list[dict], anchor_to_card: dict[str, str],
                    notes: list[str]) -> dict:
    slug = raw["slug"]
    items, free_text = parse_top_list(raw["selection"]["html"])
    ...
    for gi, item in enumerate(items, 1):
        ...
        groups.append({"index": gi, "count": head["count"], "kind": "list",
    ...
    for e in entries:
        s = named_by_card.setdefault(e["datacardId"], set())
        s |= {norm_weapon(w) for w in e["fixedWeapons"]}
        for o in e["loadouts"]:
            s |= {norm_weapon(w) for w in o["weapons"]}
    ...
    constraints, footnotes = parse_constraints(free_text, slug)
    ...
    if len(leaders) == 1:
        ...
        leader_block = {"role": None, "datacardId": None,
                        "count": next((g["count"] for g in groups
                                       if leaders[0]["group"] == g["index"]), 1),
                        "loadouts": [], "optionGroups": [], "loadoutMode": "choice",
                        "fixedWeapons": [], "alwaysWeapons": [],
                        "choices": [e["role"] for e in leaders]}
    else:
        leader_block = {"role": None, "datacardId": None, "count": 0, "loadouts": [],
                        "optionGroups": [], "loadoutMode": "fixed",
                        "fixedWeapons": [], "alwaysWeapons": [], "choices": []}
        notes.append("no datacard in this team carries the LEADER keyword")

    leader_groups = {e["group"] for e in leaders}
    slots = sum(g["count"] for g in groups if g["index"] not in leader_groups)

    return {
        "leader": leader_block,
        "slots": slots,
        "groups": groups,
        "list": [e for e in entries if not e["isLeader"]],
