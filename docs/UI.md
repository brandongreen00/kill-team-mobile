...
phone landscape, desktop). They are the input to a visual review, not fixtures.

Regenerate them with **`pnpm ui:review`** (`tools/ui-review/capture.mjs`; add
`--only phone` for one viewport). It builds, serves `dist/` on 4173 and drives the built app
with Playwright. In a container that ships its own Chromium, point at it:
`PW_CHROMIUM=/opt/pw-browsers/chromium-1194/chrome-linux/chrome pnpm ui:review`.
...
in the numbered sequence is a state the app actually reached, with zero console errors on all
three viewports. There is no seam for injecting a GameState and there should not be one:
`App` builds its own `Store`, so a PNG here can only show a screen the app can be driven to.

The harness is keyed by the `data-screen` the topbar publishes and presses buttons by the
`data-action` each carries, not by their labels — so a screen renamed in `commandPlan` shows up
here as a missing handler rather than as a capture that silently skips it. It exits non-zero if
a viewport misses a required screen or logs a console error; a reactive window without a
numbered slot is captured opportunistically as `decision-<kind>.png`.
